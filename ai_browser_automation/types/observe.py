...
class ObserveElementSchema(BaseModel):
    ...
    action: Optional[str] = Field(None, description="Suggested action (click, fill, etc.)")
    method: Optional[str] = Field(None, description="Playwright method to use (for act observations)")
    arguments: Optional[List[Any]] = Field(default_factory=list, description="Arguments for the method")
    ...
    def validate_method(cls, v):
        """Validate Playwright method if provided."""
        if v is not None:
            supported_methods = [
...
class ActObserveResponseSchema(BaseModel):
    """Schema for act-specific observe response (single element)."""
