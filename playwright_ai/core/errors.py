"""Custom exception hierarchy for PlaywrightAI."""

from typing import Optional, Any, Dict


class PlaywrightAIError(Exception):
    """Base exception for all PlaywrightAI errors."""
...
class PlaywrightAINotInitializedError(PlaywrightAIError):
    """Raised when PlaywrightAI methods are called before initialization."""
    
    def __init__(self):
        super().__init__(
            "PlaywrightAI not initialized. Call init() before using other methods.",
            {"error_code": "NOT_INITIALIZED"}
        )


class MissingEnvironmentVariableError(PlaywrightAIError):
...
class BrowserNotAvailableError(PlaywrightAIError):
...
class PageNotAvailableError(PlaywrightAIError):
...
class ElementNotFoundError(PlaywrightAIError):
...
class ActionFailedError(PlaywrightAIError):
...
class ExtractionFailedError(PlaywrightAIError):
...
class LLMError(PlaywrightAIError):
...
class SchemaValidationError(PlaywrightAIError):
...
class CacheError(PlaywrightAIError):
...
class CDPError(PlaywrightAIError):
...
class TimeoutError(PlaywrightAIError):
...
class ConfigurationError(PlaywrightAIError):
...
class UnsupportedOperationError(PlaywrightAIError):
