    ...
    async def handle(
        ...
        try:
            ...
            if options.from_act and instruction:
                ...
                instruction = options.instruction or self._get_default_instruction(
                    options.from_act)

            # Create messages using structured prompt builders
            system_prompt = build_observe_system_prompt(
                self.llm_provider.stagehand.user_provided_instructions if hasattr(
                    self.llm_provider, 'stagehand') else None
            )
...
Only return the JSON object, no other text."""
            ...
            # Parse response
            results = self._parse_observe_response(
                response, page_info, options.from_act, options.return_action)
            ...
            if iframes and not options.iframes:
                ...
                for iframe in iframes:
                    ...
                    if node_id:
                        ...
                        encoded_id = page.encode_with_frame_id(
                            None, int(node_id))
    ...
    async def _gather_page_info(self, page: 'PlaywrightAIPage', include_iframes: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if include_iframes:
                ...
                self._log_debug(
                    "Getting accessibility tree with XPath mappings")
            ...
            # Note: camelCase from TypeScript
            xpath_map = tree_result.get("xpathMap", {})
            # Note: camelCase from TypeScript
            id_to_url = tree_result.get("idToUrl", {})
            ...
            self._log_debug(
                f"Got {len(flattened_elements)} nodes from accessibility tree")
            ...
            self._log_error(
                f"Error getting accessibility tree: {e}", error=str(e))
    ...
    def _parse_observe_response(
        ...
        try:
                        ...
                        if not isinstance(elem, dict):
                            ...
                            if isinstance(elem, (str, int)):
                                ...
                                if return_action:
                                    ...
                                    elem_dict["arguments"] = []
                                validated = ObserveElementSchema(
                                    **elem_dict) if not return_action else ActObserveElementSchema(**elem_dict)
                                elements_data.append(validated.dict())
                            else:
                                self._log_debug(
                                    f"Skipping non-dict element: {type(elem).__name__}")
                            continue
            ...
            results = []
            # This is set correctly in _gather_page_info
            ...
            for elem_data in elements_data:
                ...
                if not isinstance(elem_data, dict):
                    self._log_debug(
                        f"Skipping non-dict element data: {type(elem_data).__name__}")
                    continue
                # Use elementId from LLM response
                encoded_id = elem_data.get(
                    'elementId', elem_data.get('encodedId', ''))
                ...
                result = ObserveResult(
                    ...
                    # Add method for act observations
                    method=elem_data.get('method'),
                    # Add arguments for act observations
